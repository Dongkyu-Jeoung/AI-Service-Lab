...
이 프로젝트에는 MovieLens 100K 데이터가 이미 `ml-100k/` 폴더에 포함되어 있습니다(교육용 데모를 별도
준비 없이 바로 실행할 수 있도록 하기 위함입니다). 데이터가 없는 환경(새로 클론했는데 데이터가 빠진 경우
등)에서는 아래 방법으로 준비하세요.
...
```text
services/06-movie-recommendation-ai-advanced/ai-server/data/
└── ml-100k/
    ├── u.data
    ├── u.item
    ├── u.user
...
- `notebooks/project06_movie_recommendation_advanced.ipynb`: 데이터 로딩 셀에서 파일 존재 여부를 확인하고,
  없으면 동일한 안내를 출력한 뒤 이후 셀 실행을 중단합니다.
