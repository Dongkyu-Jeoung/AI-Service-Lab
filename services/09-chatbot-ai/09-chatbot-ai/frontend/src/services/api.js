...
export async function sendMessage(sessionId, message, temperature = 0.7) {
  const { data } = await api.post("/chat", {
    session_id: sessionId,
    message,
    temperature,
  });
  return data; // { session_id, reply, prompt_tokens, completion_tokens, total_tokens }
}
...
export async function getChatHistory(sessionId) {
  ...
  return data; // { session_id, messages: [{role, content}, ...] }
}
